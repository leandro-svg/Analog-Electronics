...
% if there is only one vsb value in the table, then 3-dim. interpolation is done:
if t.Input.vsb.nInputs < 2
  ...
  if vgs < minVgs - eps | vgs > maxVgs + eps | vds < minVds - eps | vds > maxVds ...
	+ eps | lg < t.Input.lg.init - eps | lg > t.Input.lg.final + eps
  ...
  result = (((J1 * t.Table.(par)(j,k,m) + J * t.Table.(par)(j+1,k,m))* K1 + ...
      (J1*t.Table.(par)(j,k+1,m) + J*t.Table.(par)(j+1,k+1,m)) * K) * M1 + ...
      ((J1 * t.Table.(par)(j,k,m+1) + J * t.Table.(par)(j+1,k,m+1)) * K1 + ...
      (J1 * t.Table.(par)(j,k+1,m+1) + J * t.Table.(par)(j+1,k+1,m+1)) * K) ...
      * M) / (deltaLg * t.Input.vgs.step * t.Input.vds.step);

else
  % table has four dimensions:
